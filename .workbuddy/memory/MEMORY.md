...
## 约定
- **header 架构（AppHeader.vue 统一头部）**：左区头像(用户名+状态点)+应用名隐藏+快捷操作(无审批角标)；中区搜索框绝对居中(Ctrl K)；右区消息中心+窗口控制。头像菜单=**数据驱动**（`menuItems` computed + `ICON` SVG 集，desktop 项按 `isElectron` 过滤带「桌面端」badge；2026-09-07 改，删了 特别关注/调试窗口）。9 项顺序：通知模式/修改铃声(desktop→`titlebarMenu('notify'/'ringtone')`) → 修改密码/音视频设备检测(`emit`/DeviceTestModal) → 下载中心(desktop) → 安装APP和桌面端(`/download` 链接) → 设置面板(desktop→'settings') → 本地数据(`emit('open-settings')`) → 退出登录。layout.vue 转发 @join-meeting/@dismiss/@view-comment/@change-password/@open-settings/@logout。desktop-notifier 已删全部注入 CSS（TITLEBAR_CSS 等，titlebar-inpage.js=死文件）。**勿回退**：勿恢复 TITLEBAR_CSS 注入（隐藏 .app-header 致双头错位）；勿恢复审批角标。
  - **本地数据弹窗（2026-09-07）**=`web/src/views/SettingsPage.vue`（"管理账户信息与本地存储数据"），**单一实例挂在 layout.vue**（`settingsVisible`），AppHeader 头像菜单 + SideTab「数据管理」都走 `open-settings` emit；SideTab 不再自挂 SettingsPage。
  - **通知模式入口（2026-09-07）**：头像菜单「通知模式」→ `titlebar:menu` key `notify` → main.js `openSettingsWindow('general')`（通知模式四态卡=通用面板第 2 行）；settings-window.html `switchTab()` 已加切页回顶 `#content.scrollTop=0`。勿扰(dnd)仍只在设置面板模式卡片里切，勿在 titlebar:menu 做直接切换（曾临时实现已回退）。
