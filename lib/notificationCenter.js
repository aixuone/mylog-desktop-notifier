...
function createNotificationCenter(deps) {
  const { BrowserWindow, webPreferences, getWindowRect, settingsStore, ringtoneResolver, onUnreadChange, openExternal, broadcast } = deps
  ...
  ipcMain.on('nc-open-conversation', (e, d) => {
    ...
    else if (data.conversationId) openExternal(data.conversationId)
    markRead(data.conversationId ? data.conversationId : 'all')
  })
