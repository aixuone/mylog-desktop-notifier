...
const TASK_TYPES = ['event', 'meeting', 'call', 'business', 'report', 'repair', 'find', 'more']
...
// 提醒：基于「每次执行时间」做提前提醒，三种模式：none / 5min / 30min / 1day(+dayTime)
function normalizeReminder(r) {
  if (!r || typeof r !== 'object' || r.enabled === false) return { enabled: false, mode: 'none', dayTime: '09:00' }
  if (r.mode === '5min' || r.mode === '30min' || r.mode === '1day') {
    return { enabled: true, mode: r.mode, dayTime: normalizeTime(r.dayTime) || '09:00' }
  }
  ...
  return { enabled: true, mode, dayTime: normalizeTime(r.dayTime) || '09:00' }
}
